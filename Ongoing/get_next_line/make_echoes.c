...
/*TLDR; This program requires gnl. It uses it to read each line from the selected
file so it can be printed while simultaneously formated in a way that the file can
be converted into a makefile rule. The result of running this program with the name 
of a file present in the directory, should be an output on the terminal that can be
copy and pasted into the desired makefile. Enjoy <3*/
...
int main( int arc, char **wordy)
{
	...
	if (arc != 2)
		return (write(1, "No la das mrk: Incorrect argument count", 29), 1);
	...
	write(1, ":\n", 2);
	write(1, "\t@printf \"$(NEOR) Creating ", 27);
	guide = 0;
	while (wordy[1][guide])
			write(1, &wordy[1][guide++], 1);
	write(1, "... $(RESET)\"\n\t@printf \"$(MINT)🦝\\n\\n\"\n\t@> ", 45);
