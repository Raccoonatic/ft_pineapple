...
/*   Updated: 2025/11/25 13:54:32 by lde-san-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	sl_kill_the_game(t_game *game, int good, int err_cd, int exit_cd)
{
	...
	if (good == 1 && game -> victory == 0)
		sl_fail(err_cd, exit_cd, LIME"Game Over. "MINT"But in a good way");
	if (good == 1 && game -> victory == 1)
	{
		racc_print(1, BABY"Total Moves:"MINT" %d \n", game -> moves);
		racc_print(1, LIME"Victory!. ");
		sl_fail(err_cd, exit_cd, MINT"Thank you for playing my game <3\n");
	}
	if (good == 0)
	{
		if (err_cd == 3)
